...
public final class PouchInventoryAccess {
    ...
    public static List<PouchStackReference> getDeterministicPouchReferences(Inventory inventory) {
        int accessoriesBeltSlots = PouchSlotAccess.getAccessoriesBeltSlotCount(inventory.player);
        int curiosSlots = PouchSlotAccess.getCuriosSlotCount(inventory.player);
        ...
        for (int slot = 0; slot < accessoriesBeltSlots; slot++) {
            if (PouchData.isPouchStack(PouchSlotAccess.getAccessoriesBeltStack(inventory.player, slot))) {
                references.add(PouchStackReference.accessoriesBelt(slot));
            }
        }

        for (int slot = 0; slot < curiosSlots; slot++) {
            if (PouchData.isPouchStack(PouchSlotAccess.getCuriosStack(inventory.player, slot))) {
    ...
    public static ItemStack getPouchStack(Inventory inventory, PouchStackReference reference) {
        ...
        if (reference.type() == PouchStackReference.Type.ACCESSORIES_BELT) {
            return PouchSlotAccess.getAccessoriesBeltStack(inventory.player, reference.slot());
        }

        return PouchSlotAccess.getCuriosStack(inventory.player, reference.slot());
    }
    ...
    public static void commitPouchStack(Inventory inventory, PouchStackReference reference, ItemStack stack) {
        ...
        if (reference.type() == PouchStackReference.Type.ACCESSORIES_BELT) {
            PouchSlotAccess.setAccessoriesBeltStack(inventory.player, reference.slot(), stack);
        } else {
            PouchSlotAccess.setCuriosStack(inventory.player, reference.slot(), stack);
        }
        inventory.setChanged();
    }
